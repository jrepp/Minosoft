...
object FabricFunctionalityCatalog {
    ...
    val DISTANT_HORIZONS = listOf(
        ...
        partial("lod-rendering", "Distant terrain rendering", FabricFunctionalityArea.RENDERING, "The default unified renderer consumes native, generated, restored, and schema-v2 network vertical pages, derives bounded quadtree parents, builds true semantic vertical faces per selected page through the shared CPU service, and publishes region/material/view batches with independent main/shadow screen-space selection. It retains top-only input only as the coherent legacy rollback and v1 migration tier. Bounded CPU/byte admission, retry/quarantine, last-known-good shader reload, and atomic solid/water replacement are implemented and have one Apple OpenGL Bliss resource-baseline run; diverse geometry, seam-motion, checked-pixel, scaling, and cross-driver qualification remain.", "TerrainRuntimeSelection / DistantTerrainInterop / DistantTerrainRenderer / DistantHierarchicalTerrainRuntime / DistantPageHierarchyIndex / DistantPageMesher"),
        ...
        partial("lod-world-generation", "Distant world generation", FabricFunctionalityArea.WORLD, "Local authority produces detached partial vertical pages without publishing native chunks. The managed Fabric server admits bounded base-page work, deduplicates shared FULL-chunk futures, validates player/world epochs at admission and completion, and cancels queued/in-flight publication on dimension change or disconnect. Live TPS qualification remains open.", "DistantUnexploredGenerator / DistantHorizonsLodServer"),
        partial("lod-networking", "Distant terrain networking", FabricFunctionalityArea.NETWORKING, "The advertised schema-v2 channel carries connection/world epoch, normalized level, request ID, bounded page keys/detail, source revision, and bounded page payloads. The client atomically rejects wrong-world, unknown, unrequested, duplicate, unsupported-detail and stale responses; v1 remains readable but is not advertised by the managed server.", "DistantTerrainProtocolV2 / DistantTerrainRequestTracker / DistantLodNetworkClient / DistantHorizonsLodServer"),
