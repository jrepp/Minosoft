...
object FabricFunctionalityCatalog {
    ...
    val DISTANT_HORIZONS = listOf(
        mapped("chunk-events", "Chunk lifecycle ingestion", FabricFunctionalityArea.WORLD, "Native chunk create and update events capture immutable 16x16 surface-height/material tiles plus opaque fluid-bed depth without retaining live chunks.", "DistantHorizonsLodController / FabricChunkEvents"),
        mapped("block-mutation-events", "Block mutation ingestion", FabricFunctionalityArea.WORLD, "Single and batched native block mutations refresh only the affected detached tile columns after the world mutation boundary.", "DistantHorizonsLodController / FabricBlockMutationEvents"),
        ...
        mapped("client-payload-channels", "Managed-server LOD payloads", FabricFunctionalityArea.NETWORKING, "One bounded advertised source-native channel transfers explicit missing tile requests and immutable responses; upstream DH payload types do not link.", "DistantLodProtocol / FabricClientPayloadChannels"),
        ...
        mapped("distant-terrain-lod", "Explored distant terrain", FabricFunctionalityArea.RENDERING, "The exact 2.4.4-b adapter turns detached explored-surface tiles into graph-visible solid, water, and shadow routes outside the native near-terrain seam.", "DistantHorizonsRendererHook / DistantLodMeshPlanner"),
        mapped("lod-storage", "LOD storage", FabricFunctionalityArea.WORLD, "A bounded access-ordered store retains detached tiles after native unload, and a versioned palette-encoded GZIP database atomically persists them per connection/world identity across sessions. The source-native format intentionally does not claim compatibility with or migrate upstream DH databases.", "DistantLodTileStore / DistantLodPersistence / DistantLodPersistenceWriter"),
        partial("lod-rendering", "Distant terrain rendering", FabricFunctionalityArea.RENDERING, "A graph-visible renderer aggregates explored, locally generated, persisted, and network-received detached tiles on one stable 4x4 base lattice, adaptively refines relief and water boundaries to 2x2 or exact columns, stitches unequal adaptive transitions with coalesced skirts capped at 32 blocks, separates skirt-free water beds, and yields only to uploaded native meshes. The former 4x4/8x8 distance transition remains disabled until a morphing transition mesh is qualified. Cross-chunk hierarchical octrees remain unmapped.", "DistantHorizonsRendererHook / DistantLodMeshPlanner"),
        ...
        partial("lod-world-generation", "Distant world generation", FabricFunctionalityArea.WORLD, "Local-authority sessions use the existing authoritative generator to produce detached tiles without publishing native chunks. A managed Fabric server can intentionally generate one requested unexplored chunk per player per second. Upstream DH generator overrides are not executed.", "DistantUnexploredGenerator / DistantHorizonsLodServer"),
        partial("lod-networking", "Distant terrain networking", FabricFunctionalityArea.NETWORKING, "An advertised, versioned, bounded source-native channel transfers explicit missing tile positions from the managed Fabric server with radius, queue, payload, palette, and client backpressure limits. The upstream DH network protocol is not decoded.", "DistantLodProtocol / DistantLodNetworkClient / DistantHorizonsLodServer"),
    ...
    val IRIS = listOf(
        ...
        partial("shader-settings", "Shader settings screen", FabricFunctionalityArea.OPTIONS, "A persisted source-native form selects packs, applies inherited profiles and profile-driven program suppression, exposes validated boolean/enumerated defines, uses stepped controls for authored sliders, and maps authored option order/subscreens into native categories. Exact Iris column-grid presentation, localized labels/tooltips, and the remaining property/custom-input grammar remain.", "IrisPresentationController / ShaderPackSettings / ShaderPackOption"),
