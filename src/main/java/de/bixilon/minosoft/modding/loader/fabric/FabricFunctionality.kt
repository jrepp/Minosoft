...
object FabricFunctionalityCatalog {
    ...
    val ENTITY_MODEL_FEATURES = listOf(
        ...
        partial("cem-expressions", "CEM expression animation", FabricFunctionalityArea.CONTENT, "A bounded, reflection-free per-instance runtime applies ordered part transforms, visibility, persistent var/varb state, aliases, and initial entity/render inputs; complete OptiFine variables and exact absolute part-property semantics remain.", "CemExpressionEvaluator / CemExpressionManager"),
        ...
        partial("cem-reload", "CEM generation reload", FabricFunctionalityArea.RELOAD, "The render-thread content reload reparses, re-bakes, uploads, and atomically swaps CEM model/entity lookups. A rejected candidate preserves the active generation, and retired models defer GPU disposal until their final retained instance closes. Reload currently reuses the uploaded texture array; new texture keys and repeated real-GL accounting remain.", "ReloadCommand / SkeletalLoader.reloadContentFidelity / ContentGenerationStore"),
    ...
    val ENTITY_TEXTURE_FEATURES = listOf(
        ...
        partial("entity-texture-context", "Entity texture context", FabricFunctionalityArea.CONTENT, "Rules consume biome/weather/name/height/time/health, team, profession/type, color/tame/movement state, and bounded nested entity NBT through string, wildcard, regex, boolean, numeric, range, and custom predicates; complete ETF parity remains.", "EntityTextureContextFactory"),
        partial("entity-texture-variants", "Entity texture variants", FabricFunctionalityArea.RENDERING, "Deterministic stable variant selection, finite material discovery, generation-owned caching, and retained skeletal material binding exist; feature layers and player textures remain incomplete.", "EntityTextureRuleSet / SkeletalLoader"),
        partial("entity-texture-materials", "Emissive and blinking materials", FabricFunctionalityArea.RENDERING, "Deterministic base, emissive, blink, and blink-emissive frames reach retained skeletal entities; the emissive pass is full-bright and restores render state. Player and feature-layer passes plus visual parity remain.", "EntityTextureMaterial / SkeletalFeature"),
        partial("entity-texture-reload", "Entity texture generation reload", FabricFunctionalityArea.RELOAD, "The render-thread content transaction swaps ETF catalogs and pre-baked skeletal material meshes together. Each old model leases its exact catalog/cache through final retirement, while a missing texture rejects the candidate. Texture-array replacement, feature/player surfaces, and repeated real-GL disposal proof remain.", "SkeletalLoader.reloadContentFidelity / BakedSkeletalModel.contentLease"),
    ...
    val GECKOLIB = listOf(
        ...
        partial("gecko-reload", "GeckoLib generation reload", FabricFunctionalityArea.RELOAD, "Geometry and matching clips are rebuilt, uploaded, and atomically swapped on the render thread; rejected candidates preserve the active lookup, and retired GPU meshes survive retained instances until final release. Controller migration, new texture-array entries, and repeated real-GL accounting remain.", "SkeletalLoader.reloadContentFidelity / BakedSkeletalModel.retire"),
