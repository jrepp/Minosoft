...
object FabricFunctionalityCatalog {
    ...
    val ENTITY_MODEL_FEATURES = listOf(
        ...
        partial("cem-expressions", "CEM expression animation", FabricFunctionalityArea.CONTENT, "A bounded, reflection-free per-instance runtime applies ordered part transforms, visibility, persistent var/varb state, aliases, entity/render inputs, and raw nbt(key,query) predicates through the shared ETF matcher; complete live variable, diagnostic, and exact absolute part-property semantics remain.", "SkeletalExpression / CemExpressionEvaluator / CemExpressionManager / EntityTextureConditions"),
        partial("cem-render-binding", "Entity renderer binding", FabricFunctionalityArea.RENDERING, "Neutral geometry binds to retained skeletal transforms, direct textures, UVs, and matching animal/humanoid IDs. Native parts survive targeted replacement, while attach roots use isolated child transforms; broader entity/feature coverage and visual parity remain.", "SkeletalModelBinder / SkeletalModelComposer / SkeletalLoader"),
    ...
    val ENTITY_TEXTURE_FEATURES = listOf(
        ...
        partial("entity-texture-context", "Entity texture context", FabricFunctionalityArea.CONTENT, "Rules consume biome tags, weather/name/height/time/health, pack-scoped active-mod IDs, team, profession/type, general variants, color/tame/movement state, equipment/items, genes, inventory/jump/movement attributes, predicate-gated vertical block IDs, and bounded entity/client-player/vehicle NBT. Shared NBT predicates cover existence, inversion, integer ranges, wildcard list paths, raw/string, wildcard, and bounded regex forms. Historical blockSpawned, regional difficulty, and complete block-state/tag grammar remain.", "EntityTextureContextFactory / EntityTextureConditions / EntityTextureRuntimeEnvironment"),
