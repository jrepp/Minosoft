...
object FabricFunctionalityCatalog {
    ...
    val SODIUM = listOf(
        partial("chunk-render-scheduling", "Chunk render scheduling", FabricFunctionalityArea.PERFORMANCE, "The exact Sodium adapter selects the sole terrain backend and owns the scheduling call boundary, but delegates scheduling behavior to Minosoft's chunk core.", "TerrainBackendRegistry / SodiumTerrainBackend"),
        partial("terrain-backend", "Terrain backend ownership", FabricFunctionalityArea.RENDERING, "Sodium is the sole graph-visible terrain owner while active and controls preparation, transfer, visibility, and submission calls. Mesh generation and the packed vertex ABI still delegate to Minosoft core code rather than upstream Sodium algorithms.", "TerrainBackendRegistry / SodiumTerrainBackend"),
        ...
        mapped("biome-blend", "Biome blend", FabricFunctionalityArea.OPTIONS, "Radius and dependency-driven algorithm controls map to native biome blending.", "rendering.biome.blending"),
    ...
    val IRIS = listOf(
        partial("shader-pack-loader", "Shader pack loader", FabricFunctionalityArea.RENDERING, "The settings bridge discovers directory/zip packs and the native planner compiles its supported paired terrain/shadow/composite subset; broad OptiFine compatibility remains.", "IrisPresentationController / IrisShaderPackPlanner"),
        partial("shader-pipeline", "Shader rendering pipeline", FabricFunctionalityArea.RENDERING, "With MINOSOFT_SHADER_PACK set, the exact Iris adapter plans and transactionally compiles paired terrain/shadow/composite shader-pack programs, publishes their typed plan, and owns terrain plus world-composite program selection. Broader OptiFine transformation and scene-program coverage remain.", "ShaderPipelineRegistry / IrisShaderPackPlanner / IrisWorldShaderPipeline"),
        partial("shadow-maps", "Shader shadow maps", FabricFunctionalityArea.RENDERING, "A shader-pack shadow program declares a first-class graph view and owns a 1024x1024 color/depth target; terrain reuses prepared meshes for the auxiliary submission. Cascades, pack-configured target sizing, shadow entities, and complete matrices remain.", "iris:shaderpack/shadow / IrisWorldShaderPipeline"),
        partial("shader-settings", "Shader settings screen", FabricFunctionalityArea.OPTIONS, "A persisted source-native form selects packs and exposes validated boolean/enumerated shader define options; the complete Iris option grammar remains.", "IrisPresentationController / ShaderPackOption"),
        ...
        mapped("resource-reload-events", "Live shader reload", FabricFunctionalityArea.RELOAD, "Shader-pack parsing, supported-program validation, program compilation, target creation, and publication form one last-known-good generation; failed candidates leave the active pipeline selected.", "IrisShaderPipelineController.reload / ShaderPipelineRegistry"),
    )
    ...
    val GECKOLIB = listOf(
        ...
        partial("animation-controllers", "Animation controllers", FabricFunctionalityArea.API, "The source-native API supports stable content-identity routes for entities, block entities, items, and armor plus controller factories, named predicate controllers, playback/restart/stop, transitions, concurrent replace/add layers, expression data, state-driven animation speed/easing overrides, triggerable animation preemption/base reload, typed keyframe handlers, generation-owned animatable caches, owner-scoped easing/loop/event/render-layer listeners, bounded RawAnimation default/play-once/hold/loop/wait/repeat queues with delta carry, stable finished identity, current-stage queries, explicit reset, and compatible state migration without event replay. Generic objects have typed data tickets, first-tick/update state, controller triggers, compatible snapshots, and instanced or bounded singleton cache ownership. Dependent-mod validation and GeckoLib/Mojang binary APIs remain.", "GeckoLibControllerBindingRegistry / GeckoLibModelRouteRegistry / GeckoLibControllerSet / GeckoLibControllerSetSnapshot / GeckoLibRawAnimation / GeckoLibAnimatableManager / GeckoLibInstancedAnimatableInstanceCache / GeckoLibSingletonAnimatableInstanceCache / GeckoLibEasingRegistry / GeckoLibLoopTypeRegistry / GeckoLibRuntimeEvents"),
        partial("gecko-rendering", "Geo model rendering", FabricFunctionalityArea.RENDERING, "Matching geometry and animation documents cross the retained-skeletal bridge. Identity-routed multi-controller sets render entities, routed block entities, world/item-display and first-person held items, plus routed armor slots; generation-baked opaque/translucent/additive texture passes evaluate bounded state predicates and restore render state. Generic non-rendered objects use the headless manager facade. GUI item views, exact armor-to-parent-bone fitting, dependent-mod validation, and visual parity remain.", "ContentFidelityLoader / SkeletalModelBinder / GeckoLibAnimationManager / GeckoLibModelRouteRegistry / GeckoLibBlockEntityRenderer / ItemFeature / HeldItemRenderer / GeckoLibArmorFeature"),
        partial("gecko-events", "Sound, particle, and custom keyframes", FabricFunctionalityArea.GAMEPLAY, "Typed per-controller handlers retain exact sound, particle locator/pre-effect, and custom instruction data. Bounded entity, block-entity, item, and armor keyframes defer until finalized transforms, play native positional sounds, spawn registered particle factories with locator support, and dispatch owner-scoped listeners. Dependent-mod and visual parity remain.", "GeckoLibControllerKeyframeEvent / GeckoLibEventPlayback / GeckoLibEntityEventConsumer / GeckoLibBlockEntityRenderer / GeckoLibRuntimeEvents"),
        partial("gecko-reload", "GeckoLib generation reload", FabricFunctionalityArea.RELOAD, "Geometry, matching clips and render-layer meshes, plus refreshed/new static texture buckets are rebuilt, uploaded, and atomically swapped on the render thread; rejected candidates preserve active texture/model lookups, retired GPU meshes and texture coordinates survive retained instances until final release, dead slots compact without renumbering live meshes, compatible entity/item/armor controller state migrates without event replay, and routed block-entity section caches rebuild against the new generation. Repeated real-GL accounting remains.", "SkeletalLoader.reloadContentFidelity / ChunkRenderer.invalidate / ItemFeature.reloadContentModel / GeckoLibArmorFeature / StaticTextureArrayUpdate / StaticTextureSlotOwnership / GeckoLibControllerSetSnapshot / BakedSkeletalModel.retire"),
    ...
    val NATURALIST = listOf(
        mapped("skeletal-content", "Naturalist animal models", FabricFunctionalityArea.CONTENT, "The exact adapter mounts the verified artifact and owns 31 entity routes across 24 Gecko geometry identities.", "ExternalAssetProviders / GeckoLibEntityModelRegistry"),
        partial("animal-animations", "Animal animations", FabricFunctionalityArea.RENDERING, "Naturalist RP animation documents are ingested and a deterministic idle/movement controller drives retained models; upstream controller predicates, layered poses, and exact visual parity remain.", "GeckoLibControllerBindingRegistry"),
        partial("animal-textures", "Animal textures", FabricFunctionalityArea.RENDERING, "Direct textures and deterministic first variants render from the artifact; per-entity skin state, emissive layers, and the detached lizard-tail model remain.", "SkeletalLoader / ExternalAssetProviders"),
