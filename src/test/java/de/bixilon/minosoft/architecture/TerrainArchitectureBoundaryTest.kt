...
class TerrainArchitectureBoundaryTest {
    ...
    fun `selected near region path keeps transactional storage and both draw capabilities`() {
        ...
        assertTrue("batch.packet.groups" in device)
        assertTrue("group.indexCount" in device && "group.indexByteOffset" in device && "group.baseVertex" in device)
        assertTrue("OpenGlTerrainDrawPacketCache" in device && "NativeDrawPacketGroup" in device)
