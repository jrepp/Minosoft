...
| Verified | The Creative creature catalog derives candidates from the active session entity registry and exposes only identifiers with an actual baked skeletal entity route; it does not duplicate version catalogs or treat diagnostic fallback geometry as species support. | `CreativeCreatureCatalogElement`, `SkeletalLoader.previewModel`, focused loader tests, and [creature catalog evidence](../evidence/2026-07-26-creative-creature-catalog.md). |
...
| Verified | A session world's monotonic terrain epoch advances at the existing world-reset boundary even when no chunks are loaded. Near build page identities capture it, so completions from a cleared/replaced world fail the centralized identity check before publication. | `World.terrainEpoch`, `World.clear`, `TerrainWorldEpochTest`, `ChunkMeshingQueue`, and the [terrain consolidation implementation start](../evidence/2026-07-30-terrain-consolidation-start.md). |
