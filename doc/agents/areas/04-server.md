...
| Verified | The launcher can run a pinned Minecraft 1.20.4/Fabric Loader 0.19.3 server on Java 25 with an owned bridge that exposes status, player/world state, loaded-only blocks/AOI, and Fabric loader diagnostics through `debug-core`. Loader 0.15.11 is not a valid Java 25 runtime because its ASM/Mixin stack cannot read class-file major version 69. | `debug-server-fabric/`, `Play.prepareFabricServer`, [Java 25 baseline](../evidence/2026-07-30-java-25-baseline.md), and [debug control-plane evidence](../evidence/2026-07-22-debug-control-plane.md). |
| Verified | The managed Fabric bridge can advertise a versioned source-native distant-LOD channel and authoritatively generate requested unexplored chunks. Requests are payload/count/radius checked, each player queue is capped at 128 tiles, generation is limited to one tile per player per 20 server ticks, and responses carry bounded material palettes. This is a Minosoft acceptance/managed-server protocol, not the upstream Distant Horizons wire protocol. | `DistantHorizonsLodServer`, `DistantLodProtocol`, bridge codec tests, live request/tile/queue counters, and [DH/Bliss integration evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md). |
...
| Verified | Server support-pack selection is explicit through `MINOSOFT_SERVER_MODPACK`, defaulting to `fabric-stack`. The focused DH/Bliss acceptance server therefore installs only that pack's server-compatible Fabric API, Terralith, Tectonic, and managed debug bridge instead of inheriting unrelated Naturalist registry content. | `Play.serverModpackName`, `Play.prepareFabricServer`, focused pack manifests, live loader inventory, and [DH/Bliss integration evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md). |
