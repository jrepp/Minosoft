...
| Verified | Renderer-bootstrap and credits integration tests are independent of the local Mojang index and client JAR. `LocalMinecraftAssets` retains the ordinary profile-controlled index/JAR compatibility lane while allowing those tests to select neither source without mutating the profile. They mount exact Minosoft-authored model/font fixtures plus an inert generic fallback through the caller-priority seam. An explicitly configured out-of-source content-forge tree is mounted ahead of the generic layer and is independently checked for bounded paths, provenance hashes, JSON objects, decodable PNG bounds, and Blockbench producer records. The complete Minosoft-authored stand-in remains the target; the generic fallback is only the current bootstrap boundary. | `AssetsLoader.priorityAssets`, `LocalMinecraftAssets`, `OfflineTestAssets`, `OfflineFallbackAssetsManager`, `ContentForgeAssetsIT`, and [offline integration asset evidence](../evidence/2026-08-10-offline-integration-assets.md). |
| Verified | The optional `survival-authored` overlay mounts after bundled resource packs and before user archives. Generated diffuse textures, normal/specular companions, models, and blockstates publish together; the broad approximation catalog remains lower priority. Actual Small Survival asset hashes matched all 143 winning files, and all 24 authored blockstates baked across their registered properties. | `content-stacks/standalone.json`, `ContentStackManifestTest`, `ContentForgeBlockLoadIT`, and [content remediation evidence](../evidence/2026-09-07-small-survival-content-fix.md). |
...
| Verified | Compiler warnings are build failures across every JVM project: Kotlin uses `allWarningsAsErrors`, while Java uses `-Xlint:all -Werror`. A forced Java 25.0.4 rebuild of main, unit, integration, render-contract, debug-core, play-util, and Fabric bridge source sets completed without diagnostics. | Root Gradle JVM conventions and the [Java 25 baseline](../evidence/2026-07-30-java-25-baseline.md). |
