...
| Verified | The dependency-clean terrain contract layer freezes version-one capability, structured rejection, atomic complete-snapshot, and capability-scoped response schemas. Page listing requires a world epoch, strict bounded area or prefix selector, explicit 1–1024 result cap, deterministic order, and an opaque cursor bound to the selector and diagnostic generation; stale, malformed, and mismatched cursors reject explicitly. Canonical JDK-only JSON and checked golden fixtures cover capabilities, rejection, provider/identity/page/coverage state, and cross-generation snapshot consistency. The live `render.terrain-diagnostics` operation performs one render-thread capture and currently advertises only `ATOMIC_SNAPSHOT`, `PROVIDER_DESCRIPTOR`, and `STRUCTURED_REJECTION`; world, pipeline/material, page/coverage, residency, and submission subjects return explicit `MISSING_CAPABILITY` responses until their production owners exist. | `terrain/runtime/diagnostic`, `TerrainDiagnosticAssembler`, `TerrainDiagnosticDebugOperation`, `ClientDebugChannel`, focused tests, checked JSON fixtures, and the [terrain consolidation implementation start](../evidence/2026-07-30-terrain-consolidation-start.md). |
...
| `render.terrain-diagnostics` | client | render queue | one atomic provider capture encoded through the version-one terrain response schema; unavailable consolidated world/pipeline/page/coverage/residency/submission subjects reject with `MISSING_CAPABILITY` |
...
3. Expand the live provider-only terrain diagnostic response as consolidated
   world epoch, complete pipeline/material generation, bounded page/coverage,
   residency, and submission owners enter production. Preserve one
   render-thread capture and explicit capability advertisement; do not infer or
   fabricate unavailable generation identities. Version the remaining state
   DTOs; frame and terrain preparation/submission already expose bounded
   median/p95 windows. Add tick histograms and driver-side GPU object counts
   without making the snapshot unbounded.
