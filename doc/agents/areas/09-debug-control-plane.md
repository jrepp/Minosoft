...
| Verified | Root `metrics.snapshot` exposes compact production terrain gauges, outcomes, bytes, worker utilization, and phase percentiles. `render.substrate.terrain.productionRuntime` adds the fixed histogram bounds/buckets and requested/started/duplicate-suppressed counters for every fixed build cause, while `render.terrain-telemetry` toggles recording on the render thread for identical-workload A/B acceptance only at an idle build/upload boundary. Disabling recording performs no phase clock reads or counter/gauge mutation. | `ClientDebugChannel`, `TerrainPerformanceTelemetry`, focused telemetry tests, and [terrain runtime architecture](../../design/terrain-runtime-architecture.md). |
| Verified | The dependency-clean terrain contract layer freezes version-one capability, structured rejection, atomic complete-snapshot, and capability-scoped response schemas. Page listing requires a world epoch, strict bounded area or prefix selector, explicit 1–1024 result cap, deterministic order, and an opaque cursor bound to the selector plus selected domain's page-registry generation; stale, malformed, and mismatched cursors reject explicitly without making ordinary frame advancement or unrelated-domain publication stale the cursor. Canonical JDK-only JSON and checked golden fixtures cover capabilities, rejection, provider/identity/page/coverage state, scheduler/mailbox state, and cross-generation snapshot consistency. The live legacy `render.terrain-diagnostics` snapshot and the exact bounded summary/pages/coverage/page/flush-idle/compare/fault operations capture or derive their state on the render thread and advertise every operation/schema version. Page detail includes real source, revisions, ranges, main/shadow visibility, failure, coverage, and artifact digest. Named idle waits use real build/upload/retirement predicates; active draw fences remain visible but do not masquerade as upload debt. Deterministic comparison publishes neither candidate. Acceptance-only typed upload rejection is one-shot, returns a restoration token, preserves the active page, retries, and is invalidated on generation replacement or disconnect. | `terrain/runtime/diagnostic`, `TerrainDiagnosticAssembler`, `TerrainDiagnosticDebugOperation`, `TerrainProductionDiagnosticCapture`, `ClientDebugChannel`, focused tests, checked JSON fixtures, the [live diagnostic/fault record](../evidence/2026-07-31-terrain-diagnostic-operations.json), and the [terrain consolidation implementation start](../evidence/2026-07-30-terrain-consolidation-start.md). |
...
| Verified | OpenGL render-graph passes and individual Iris graphics/compute programs now have asynchronous `GL_TIME_ELAPSED` telemetry. One-in-four sampling is deterministically staggered by pass, the pending query pool is capped at 256, the cross-reload pass registry is capped at 512, results are polled only after availability, and query names are pooled and resource-accounted. `render.substrate.gpuResources.passTimings` reports the sampling rate, pending/dropped counts, and fixed-bucket sample/total/min/max/median-upper-bound/p95-upper-bound records without exposing OpenGL names or blocking for results. A matched live run accumulated more than 1,200 samples for each active terrain/fullscreen pass with zero dropped samples and no pending queries at the final boundary. The tuned final generation bounded translucent, opaque, cutout, and `deferred1` p95 at 4 ms; `composite1` p95 was 2 ms. | `OpenGlGpuTimer`, `RenderSystem.measureGpuPass`, `RendererPipeline`, `IrisWorldShaderPipeline`, `ClientDebugChannel.renderSubstrate`, and matched live GPU timing evidence. |
...
| `render.terrain.summary` | client | render queue | complete atomic terrain snapshot plus endpoint, frame, world, provider, shader, and layout generations |
...
0.19.3, Fabric Installer 1.1.2, Fabric API 0.97.3, and the repository-owned
...
launcher. The bridge and pack metadata keep Fabric Loader 0.15.11 as their
minimum compile-time compatibility boundary; the managed Java 25 runtime uses
0.19.3 because its newer ASM/Mixin stack can read class-file major version 69.
The bridge starts after `SERVER_STARTED`, samples server-owned state through
`MinecraftServer.execute`, and closes before server shutdown.
...
3. Extend the complete terrain diagnostic with richer owner-sourced page detail
   (source/revisions, ranges, and digest) without making page retrieval or
   hot-path state unbounded. Version any wire expansion; frame and terrain
   preparation/submission and provider-neutral per-view visibility/publication
   counts already have bounded generationed snapshots. Add tick
   histograms and driver-side GPU object counts without making the snapshot
   unbounded.
