...
| Verified | Optional DH-style unexplored generation preserves authority. Local sessions call the existing local `ChunkGenerator` into detached `ChunkBuilder` snapshots without publishing native chunks; remote sessions request the managed Fabric authority, whose bounded server tick deliberately calls `ServerWorld.getChunk` before capturing surface and water-bed columns. Both paths publish the same immutable LOD tile type outside native view distance. | `DistantUnexploredGenerator`, `DistantHorizonsLodServer`, `DistantChunkSpiral`, focused spiral/codec tests, live server generation counters, and [DH/Bliss integration evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md). |
