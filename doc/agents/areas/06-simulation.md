...
| Verified | The debug generator builds its block-state catalog over a continuous stone floor at Y=7. The floor is bounded to the catalog plus one negative-chunk spawn apron; chunks beyond that boundary remain empty, while the reserved water/light/biome canary keeps its specialized surface. | `DebugGenerator`, `DebugGeneratorTest`, Java 25 focused integration checks, and live `world.blocks.sample` verification in the `terrain-local-dojo` trajectory. |
...
| Verified | The fresh deterministic `world-diverse-medium-2026-08-01` server world uses Terralith plus Terratonic, seed `80812202647291357`, and a world-owned first-player function that centers a 2,048-block-diameter border and spawn at the safe land position `(22.5, 81, -629.5)`. A live border round-trip reported center `(22.5, -629.5)` and radius `1,024`, then restored it exactly. Graceful save plus bounded stopped-world inspection read 2,601 chunks, including 841 full chunks and zero failures, and found 14 vanilla/Terralith ocean, forest, plains, taiga, cave, Skylands Spring, and Yellowstone biomes across terrain from Y=58 through Y=244. | `server.properties`, the world's `minosoft-medium-world` datapack, `render.prepare-world-border`, and [medium diverse world evidence](../evidence/2026-08-01-medium-diverse-world.json). |
