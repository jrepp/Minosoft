...
| Verified | Integration and terrain-contract fixtures retain their committed bytes on every platform so shader/export fingerprints and canonical golden dumps survive Windows checkout. Checkout-attribute changes trigger CI. The Play utility test constructs its Java classpath wildcard after filesystem path resolution. | `.gitattributes`, `IrisShaderPackPlannerTest`, `AnimatedJavaExportFixtureTest`, `DistantVerticalPageCodecTest`, `.github/workflows/build.yml`, and `PlayUtilityTest`. |
| Observed | Animated Java fixture verification precedes the temporary rendering override, and cleanup restores rendering even when asset unloading fails. A rejected manifest cannot disable rendering for later tests. | `AnimatedJavaExportFixtureTest`. |
