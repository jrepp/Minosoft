...
| Verified | Render-substrate fixtures retain their committed bytes on every platform so shader-pack fingerprints survive Windows checkout. The Play utility test constructs its Java classpath wildcard after filesystem path resolution. | `.gitattributes`, `IrisShaderPackPlannerTest`, and `PlayUtilityTest`. |
