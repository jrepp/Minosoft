...
| Verified | The Java play utility provides expiring cross-process mutation leases, bounded hashed diagnosis bundles, atomically published stopped-world snapshots, and player-pose capture/mark/compare-and-restore checkpoints. Live server-thread save/flush, broader checkpoint fields, and command-wide lease enforcement remain target work. | `TrajectoryLeaseStore`, `TrajectoryDiagnostics`, `WorldSnapshot`, `TrajectoryCheckpointStore`, focused `play-util` tests, and the [agent trajectory tooling backlog](../backlog/agent-trajectory-tooling.md). |
