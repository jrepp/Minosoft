...
| Verified | The exact Distant Horizons adapter registers native persisted rendering, generation, storage, and multiplayer settings. Its remote path waits for an advertised source-native server capability, requests only explicit missing positions outside native view distance, retains at most 32 pending tiles, expires stale requests, and decodes bounded responses into the same detached store used by exploration and persistence. Modern Fabric payloads use the protocol packet's raw remainder rather than adding a nested byte-array length. | `DistantHorizonsOptions`, `DistantLodNetworkClient`, `DistantLodProtocol`, `FabricClientPayloadChannels.send`, focused tests, managed-server counters, and [DH/Bliss integration evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md). |
