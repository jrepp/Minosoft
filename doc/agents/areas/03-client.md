...
| Verified | The exact Distant Horizons adapter registers native persisted rendering, generation, storage, and multiplayer settings. Its remote path waits for an advertised source-native server capability, requests explicit missing positions from the immediate neighbouring chunks outward, using retained tiles rather than a square native-distance exclusion, retains at most 32 pending tiles, expires stale requests, and decodes bounded responses into the same detached store used by exploration and persistence. Modern Fabric payloads use the protocol packet's raw remainder rather than adding a nested byte-array length. | `DistantHorizonsOptions`, `DistantLodNetworkClient`, `DistantLodProtocol`, `FabricClientPayloadChannels.send`, focused tests, managed-server counters, [missing-ring regression evidence](../evidence/2026-09-07-small-survival-render-inputs-and-lod-coverage.md), and [DH/Bliss integration evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md). |
...
| Verified | The standard Minecraft F2 binding captures the final framebuffer and stores `yyyy-MM-dd_HH.mm.ss[_N].png` directly beneath the active game home's `screenshots/` directory. Capture readback stays on the render path, PNG storage stays on the I/O pool, and the existing clickable saved/delete chat response remains intact. | `DefaultKeyBindings.SCREENSHOT`, `ScreenshotTaker`, focused naming/storage tests, live injected F2 acceptance, and [screenshot/terrain evidence](../evidence/2026-07-28-screenshot-and-terrain-stability.md). |
...
| Verified | Server-announced view distance from modern initialization and runtime update packets remains authoritative across incomplete chunk delivery, movement, unload, and respawn. Protocols without an announcement retain the loaded-chunk-bounds estimate. | `InitializeS2CP`, `ViewDistanceS2CP`, `WorldView`, and `WorldViewServerDistanceTest`. |
...
| Verified | Creative creature spawning is permission-aware: the UI enables `Spawn` only when the server command tree grants `/summon`, then sends an absolute three-block-ahead request through the normal command transport without locally mutating the world. | `CreatureSpawnCommand`, `CommandNode.hasDirectChild`, focused tests, and [creature catalog evidence](../evidence/2026-07-26-creative-creature-catalog.md). |
| Verified | Creative sheep entries expose fixed woolly/sheared presets. Their command suffixes are closed enum values rather than user-provided NBT, and the world renderer gates an inflated dye-tinted wool feature from synchronized sheared state. | `CreatureSpawnVariant`, `SheepRenderer`, sheep skeletal resources, focused tests, and [creature catalog evidence](../evidence/2026-07-26-creative-creature-catalog.md). |
