...
| In progress | Complementary Unbound r5.8.1 still has an Apple OpenGL visual/runtime gate beyond the managed `TAA_MODE=0` boundary: a populated diverse world reproduced block-aligned/stippled corruption and a render-thread wedge after independent DH, light-shaft, entity, particle, and shadow A/B checks. The same pose is clean and responsive with only Iris presentation disabled. Planner and producer-completeness claims remain valid, but the current generation must not be used as a checked visual baseline. | `IrisOpenGlRenderTargets`, `IrisFrameState`, `render.substrate`, focused tests, the [terrain consolidation implementation start](../evidence/2026-07-30-terrain-consolidation-start.md), and the [root-cause test plan](../backlog/recent-terrain-visual-root-cause-test-plan.md). |
| In progress | A later built-in-presentation A/B found a separate DH ownership problem: a fully hydrated hierarchy with zero missing pages and no allocation/upload failures still drew giant foreground slabs until only DH presentation was disabled. The failure is downstream of adapter activation/network hydration and must be fixed at the near-coverage/page-span mask boundary before DH visual completion. | `DistantHorizonsDebugProvider`, `TerrainCoveragePageMask`, `DistantHierarchicalTerrainRuntime`, the [recent trajectory summary](../evidence/2026-08-01-recent-trajectory-summary.md), and the [root-cause test plan](../backlog/recent-terrain-visual-root-cause-test-plan.md). |
