...
| Verified | The exact Distant Horizons 2.4.4-b artifact activates through a source-native adapter with owned chunk/block/world/payload hooks, a bounded detached LOD store, versioned cross-session persistence, local-authority generation, advertised managed-server transfer, native settings, and separate distant solid/water/shadow routes. Every producer publishes the same immutable vertical surface/material/fluid-bed page with provenance; strict-majority parent reduction prevents minority columns from becoming full parent slabs, and the mesher closes only observed neighbouring coverage instead of inventing skirts or an exterior underside at unknown page boundaries. Near/distant ownership is represented by uploaded render-ready native chunks and a frame-pinned three-state page mask; the populated-world foreground overlap is closed by partial-page refinement and near-coverage revision-aware selection. Non-persistent presentation and bounded geometry diagnostics support exact live A/B checks. The renderer, shader, planner, and mesh builder now live in neutral terrain rendering packages; Fabric retains exact-artifact activation, settings, source ingestion, persistence/protocol translation, and registration through neutral render-source/configuration seams. Persistence hydration and an ordered bounded payload-decode drain run away from world join and the Netty event loop; restored pages become renderable only after complete-source replacement checks, and protocol receive callbacks never issue redundant cancellation writes. Its declared break against pinned Iris 1.7.2 is accepted only as one typed exact-version dependency issue because neither binary is linked. The focused pack includes Terralith/Tectonic server support, and real-driver managed-world runs have exercised persistence/native/network sources. Upstream DH database formats, generator/network protocols, binary API, and pixel-identical UI remain unmapped. | `DistantHorizonsCompatibilityAdapter`, `DistantTerrainRenderer`, `DistantTerrainRenderSource`, `DistantHorizonsDebugProvider`, `DistantDirectoryTerrainStore`, `DistantLodNetworkClient`, `DistantHorizonsOptions`, `DistantHorizonsLodServer`, `DistantPageMesher`, `modpacks/distant-horizons-bliss/`, focused tests, live diagnostics, `render.substrate`, and [terrain consolidation evidence](../evidence/2026-07-30-terrain-consolidation-start.md). |
| Verified | Iris distant scene bridges preserve the renderer's region-relative geometry contract by adding the draw-time `uPageOffset` before projection and declaring it in the exact scene-state ABI. Distant shadow collection is separately conditional on a real shadow-phase `dh_shadow` program; packs that provide only `dh_terrain`/`dh_water` no longer receive a fabricated host fallback. Schema-v2 pages retain their remote protocol epoch through request/response admission, then re-key to the monotonic client world epoch before local persistence and render publication. A fixed-pose Complementary replay removed the stacked-region panels and false shadow fallback; a restart with differing server/client epochs retained network pages and replaced the finite pale frontier with a continuous fogged silhouette. Rejected/fallback scene-bind and network-response ledgers remained empty. | `IrisLegacyShaderTransformer`, `IrisWorldShaderPipeline`, `RendererPipeline.shadowCasterEnabled`, `DistantLodNetworkClient`, focused planner/transformer/network integration tests, `render.substrate`, and [DH/Bliss region-origin evidence](../evidence/2026-07-29-distant-horizons-bliss-integration.md#region-origin-absent-shadow-and-network-epoch-follow-up). |
...
| Verified | Iris shader settings consume bounded pack-authored `shaders/lang` metadata with requested-locale plus `en_us` fallback for option comments and option/profile/screen/value labels. Nested authored screens collapse under their main-screen groups rather than producing one tab per subscreen; shader search crosses those groups, and forms with more than four categories use a full-width selector. Human-readable define/value fallbacks, explicit default values, dirty counts, `Reset all`, and post-apply session reload keep packs without complete language metadata and cross-entry profile writes understandable. Exact Iris column grids, spacer cells, and nested navigation remain partial UI parity. | `IrisShaderPackPlanner.settings`, `ShaderPackLanguage`, `ShaderPackSettings.optionGroups`, `IrisPresentationController.schema`, `SettingsFilter`, `SettingsFormMenu`, and focused planner/settings tests. |
...
| Verified | The Fabric stack's former managed default, official untouched Complementary Unbound r5.8.1, remains a qualified external shader-pack reference. The launcher stages shader packs separately from resource packs, validates the archive, forwards bounded option overrides, and preserves an explicit process-environment override. The managed Apple OpenGL baseline disables `TAA_MODE`: a same-pose live A/B isolated global framebuffer trails and whiteout to the temporal path, while the non-temporal generation retained recognizable terrain and stable main-scene routing. Temporal AA remains an opt-in fidelity boundary until that history path has a checked Apple reference. The exact archive otherwise passes a producer-complete external planner contract: all 24 non-terrain scene-state ABIs and the six entity-shadow caster states have executable pack bridges. Per-program compaction maps only the physical host texture arrays used by each terrain/scene ABI into dense shader sampler indices, while pack samplers fill the remaining sparse hardware units. This keeps Complementary's water and shadow programs below the driver's 16-fragment-sampler limit. Pack-controlled legacy shadow calls are rewritten with a balanced linear scan rather than a backtracking expression, keeping disable/restore planning bounded for deeply nested inputs. Float directives are likewise token-scanned once per expanded stage, retained only for the supported directive catalog, validated as finite, and reused across smoothing and shadow planning instead of rescanning every stage per directive. Iris owns the single shader generation and final presentation; persistent physical buffer-role alternation preserves temporal history for packs and profiles that qualify it. The active `clouds=off` property and sun-scatter specialization suppress duplicate host atmosphere draws through exact retained no-op programs. Calculated PBR consumes exact retained cuboid midpoints for entity, player, arm, block-feature, and held-item geometry; companion normal/specular delivery, temporal pixel parity, and broader driver/profile coverage remain partial. | `util/play/Play.java`, `IrisTextureArrayLayout`, `IrisOpenGlRenderTargets`, `IrisShaderPackPlanner`, `IrisLegacyShaderTransformer`, `IrisCustomUniformEvaluator`, `LocalDisplayEntityFactory`, focused tests, `render.substrate`, and [Iris pipeline evidence](../evidence/2026-07-26-iris-render-pipeline-support.md). |
| Verified | The combined `fabric-stack` now pins the exact Bliss 2.1.0 Modrinth archive as its sole managed default shader pack, reusing the same URL and SHA-512 identity as the focused DH+Bliss pack. Pack preparation and compatibility inspection select Bliss without an environment override; the exact archive crosses its dedicated DH planner contract. A live Apple OpenGL generation compiled 72 main and seven shadow scene programs with empty rejection/fallback ledgers and selected `gbuffers_water` every checked frame. A later untouched Bliss 2.1.2 A/B found and fixed the red/green mottling as an Iris custom-texture bootstrap lifetime mismatch: after Bliss first flips `colortex6`, later bloom passes now sample the render target instead of the original blue-noise custom texture. A source-anchored Bliss presentation transform increases only above-water reflective output, supplies a biome-tinted floor for crushed dark water, and retains a bounded 35 percent share of local color through underwater absorption. The exact 2.1.0 archive planner gate proves both rewrites reach their executable programs; post-audit surface/submerged pixel qualification and dedicated performance remediation remain open. | `modpacks/fabric-stack/shaderpacks/bliss.pw.toml`, `modpacks/distant-horizons-bliss/shaderpacks/bliss.pw.toml`, `Play.preparePack`, `IrisShaderPackPlanner`, `IrisBlissWaterTransformer`, focused transformer/planner tests, and [Iris pipeline evidence](../evidence/2026-07-26-iris-render-pipeline-support.md#bliss-water-presentation-follow-up). |
...
| Verified | Phase-7 distant ingestion uses the neutral normalized vertical sampler for observed chunks, fully detached/halo-lit local authority, and managed-server capture. Local preparation remains bounded by actual generated chunks per tick and clears cached/pending builders on world epoch. The schema-v2 directory store writes checksum-protected atomic dirty-page records through a bounded coalescing writer, rekeys restored pages, pins dirty keys, applies distance/recency eviction, migrates readable v1 tiles one-way without retaining a second writer, and exposes deterministic inspection. The shared v2 protocol carries connection/world epochs, normalized level, request identity, exact pages/detail/source revisions and bounded payloads; checked-in goldens pin every message. The client rejects wrong-world, unknown, unrequested, duplicate, and unsupported-detail responses without publication or connection failure; a response superseded by newer local data is consumed page-by-page without publication so the rest of its correlated response stream remains admissible. Terminal receive paths do not write redundant cancellations; timeout and lifecycle retirement still send exact correlated cancellations. Stable persistence identity includes the protocol world fingerprint, so same-address/same-name different-seed reconnects cannot alias stores. The managed server atomically validates admission/completion context, bounds and deduplicates asynchronous FULL-chunk work, shares page results safely across request epochs, and retires cancelled/disconnected/dimension-changed/failed work. Headless normalized network/store parity, live adapter parity, crash regression, 20 TPS sampling, dimension transitions, and same-name different-world reconnect gates pass. | `DistantGeneratedChunkCache`, `DistantDetachedLighting`, `DistantWorldVerticalSampler`, `DistantVerticalPageCodec`, `DistantDirectoryTerrainStore`, `DistantTerrainStoreWriter`, `DistantTerrainProtocolV2`, `DistantTerrainRequestTracker`, `DistantHorizonsLodServer`, focused lighting/codec/store/protocol/server/migration/parity/lifecycle tests, the [terrain consolidation implementation start](../evidence/2026-07-30-terrain-consolidation-start.md), and the [same-name reconnect evidence](../evidence/2026-08-01-terrain-l5-world-reconnect.json). |
...
| Verified | Blockbench/Bedrock Molang `math.*` trigonometry is degree-based without changing EMF's bare radian functions. Gecko controller play/stop/queue transitions preserve the last evaluated pose, so an outgoing expression clip is not reevaluated without `query.anim_time`. Runtime and inspection expression contexts expose both `query.anim_time`/`q.anim_time` and `query.is_moving`/`q.is_moving`, while preview supplies bounded defaults. The live Naturalist snake now animates without the former `math.cos` or transition-context crash, and the short-alias transition regression is focused-tested. This covers the source constructs observed in the pinned Naturalist animation set; it is not a full Molang language claim. | `SkeletalExpression`, `SkeletalAnimationController`, `GeckoLibAnimationState`, `GeckoLibControllerSet`, `SkeletalPreviewPlayer`, focused expression/controller/preview tests, and [Naturalist registry-sync evidence](../evidence/2026-07-26-naturalist-registry-sync.md). |
...
| Verified | Complementary Unbound r5.8.1 passes the populated diverse-world Apple OpenGL 4.1 gate twice across fresh client generations plus a five-minute soak. The adapter retains the exact managed fingerprint/options and empty fallback/rejection maps. Generic host fixes provide player-relative fullscreen matrices and program-scoped mipmap minification; the acceptance-only pass cutoff remains generation-scoped and resets with its Fabric owner. | `IrisCompatibilityAdapter`, `IrisFrameState`, `IrisOpenGlRenderTargets`, `IrisPassCutoff`, exact-pack/focused/real-GL tests, checked Iris scenarios, and [root-cause completion](../evidence/2026-08-03-recent-terrain-visual-root-cause.json). |
| Verified | The separate DH foreground ownership failure is closed downstream of adapter activation and hydration. Three-state page coverage drives partial refinement, settled full masking, and conservative missing-child fallback; fixed-camera selection includes near-coverage revision. The adapter's same-pose scenario proves the foreground fix while requiring positive distant rendering, masked pages, zero missing pages, and zero terrain failures, then restores the null presentation override and configured enabled state. | `DistantHorizonsDebugProvider`, `TerrainCoverageMasking`, `DistantPageSelector`, `DistantHierarchicalTerrainRuntime`, `terrain-dh-near-mask-diverse-medium`, and [root-cause completion](../evidence/2026-08-03-recent-terrain-visual-root-cause.json). |
